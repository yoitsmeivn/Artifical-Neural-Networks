...
class SigmoidLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        sigmoid_output = self.forward(self._cache_current)
        derivative = sigmoid_output * (1 - sigmoid_output)
        return derivative * grad_z
...
class LinearLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        self._cache_current = np.transpose(self._cache_current)
        ...
        grad_input = np.dot(grad_z,w_transpose)
        return grad_input
...
            - neurons {list} -- Number of neurons in each linear layer 
        ...
        #######################################################################
        self._layers = []
        curr_dimension = input_dim
        for curr_layer in range(len(neurons)):
            self._layers.append(LinearLayer(curr_dimension, neurons[curr_layer]))
            if activations[curr_layer] == "relu":
                self._layers.append(ReluLayer())
            elif activations[curr_layer] == "sigmoid":
                self._layers.append(SigmoidLayer())
            else:
                pass
            curr_dimension = neurons[curr_layer]
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
    ...
    def forward(self, x):
        ...
        #x is batch size and input dimension
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        #reversed for gradient for computation in opposite direction
...
class Trainer(object):
    ...
    def shuffle(input_dataset, target_dataset):
        ...
        input_shuffle = input_dataset[indices]
        target_shuffle = target_dataset[indices]
        return input_shuffle,target_shuffle
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
    ...
    def train(self, input_dataset, target_dataset):
        ...
        #######################################################################
        #train
        for epoch_time in range(self.nb_epoch):
            if self.shuffle_flag == True:
                input_dataset, target_dataset = self.shuffle(input_dataset,target_dataset)
            num_batches = len(input_dataset)//self.batch_size
            mini_batchs = [(input_dataset[i * self.batch_size:(i + 1) * self.batch_size], target_dataset[i * self.batch_size:(i + 1) * self.batch_size]) for i in range(num_batches)]
            for batch_inputs,batch_targets in mini_batchs:
    ...
    def eval_loss(self, input_dataset, target_dataset):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        data_predictions = self.network(input_dataset)
        eval_lost = self._loss_layer.forward(data_predictions,target_dataset)
        return eval_lost
